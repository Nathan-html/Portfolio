...
const Year: NextComponentType = ({ props }: any) => {
  return (
    <Box margin={'0 1rem'} userSelect={'none'}>
      <Heading as="h3" size="md" sx={{ opacity: ' 0.5', marginBottom: '1rem' }}>
        {props.year}
      </Heading>
      {props.achievements.map((achievement: Achievement, key: number) => {
        return (
          <Box key={key}>
            {key > 0 && <hr style={{ margin: '0.5rem 0' }} />}
            ...
            {achievement.place !== 'none' && (
              <Text sx={{ opacity: ' 0.5' }}>{achievement.place}</Text>
            )}
          </Box>
        )
      })}
